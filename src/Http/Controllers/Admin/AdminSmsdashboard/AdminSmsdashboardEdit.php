...
/**
 * Smsdashboard 수정 컨트롤러
 * 
 * 기존 Smsdashboard 정보를 수정하는 폼 표시 및 처리를 담당합니다.
 * Livewire 컴포넌트(AdminEdit)와 Hook 패턴을 통해 동작합니다.
 * 
 * @package Jiny\Admin\App\Http\Controllers\Admin\AdminSmsdashboard
 * @since   1.0.0
 */
class AdminSmsdashboardEdit extends Controller
{
    /**
     * JSON 설정 데이터
     *
    ...
    {
        // 데이터베이스에서 데이터 조회
        $tableName = $this->jsonData['table']['name'] ?? 'admin_smsdashboards';
        ...
        if (! $data) {
            if (isset($this->jsonData['route']['name'])) {
                ...
                $redirectUrl = '/admin/smsdashboards';
            }

            return redirect($redirectUrl)
                ->with('error', 'Smsdashboard을(를) 찾을 수 없습니다.');
        }
