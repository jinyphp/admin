...
class AdminIptrackingShow extends Controller
{
    ...
    {
        // 데이터베이스에서 데이터 조회
        $tableName = $this->jsonData['table']['name'] ?? 'admin_iptrackings';
        ...
        if (! $data) {
            if (isset($this->jsonData['route']['name'])) {
                ...
                $redirectUrl = '/admin/iptrackings';
            }

            return redirect($redirectUrl)
                ->with('error', 'Iptracking을(를) 찾을 수 없습니다.');
        }
