...
/**
 * Webhookdashboard 수정 컨트롤러
 * 
 * 기존 Webhookdashboard 정보를 수정하는 폼 표시 및 처리를 담당합니다.
 * Livewire 컴포넌트(AdminEdit)와 Hook 패턴을 통해 동작합니다.
 * 
 * @package Jiny\Admin\App\Http\Controllers\Admin\AdminWebhookdashboard
 * @since   1.0.0
 */
class AdminWebhookdashboardEdit extends Controller
{
    /**
     * JSON 설정 데이터
     *
    ...
    {
        // 데이터베이스에서 데이터 조회
        $tableName = $this->jsonData['table']['name'] ?? 'admin_webhookdashboards';
        ...
        if (! $data) {
            if (isset($this->jsonData['route']['name'])) {
                ...
                $redirectUrl = '/admin/webhookdashboards';
            }

            return redirect($redirectUrl)
                ->with('error', 'Webhookdashboard을(를) 찾을 수 없습니다.');
        }
        ...
        // JSON 파일 경로 추가
        $jsonPath = __DIR__.DIRECTORY_SEPARATOR.'AdminWebhookdashboard.json';
