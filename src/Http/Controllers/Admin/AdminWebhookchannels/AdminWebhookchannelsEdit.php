...
namespace Jiny\Admin\Http\Controllers\Admin\AdminWebhookchannels;
...
/**
 * Webhookchannels 수정 컨트롤러
 * 
 * 기존 Webhookchannels 정보를 수정하는 폼 표시 및 처리를 담당합니다.
 * Livewire 컴포넌트(AdminEdit)와 Hook 패턴을 통해 동작합니다.
 * 
 * @package Jiny\Admin\App\Http\Controllers\Admin\AdminWebhookchannels
 * @since   1.0.0
 */
class AdminWebhookchannelsEdit extends Controller
{
    /**
     * JSON 설정 데이터
     *
    ...
    {
        // 데이터베이스에서 데이터 조회
        $tableName = $this->jsonData['table']['name'] ?? 'admin_webhookchannels';
        ...
        if (! $data) {
            if (isset($this->jsonData['route']['name'])) {
                ...
                $redirectUrl = '/admin/webhookchannels';
            }

            return redirect($redirectUrl)
                ->with('error', 'Webhookchannels을(를) 찾을 수 없습니다.');
        }
        ...
        // JSON 파일 경로 추가
        $jsonPath = __DIR__.DIRECTORY_SEPARATOR.'AdminWebhookchannels.json';
