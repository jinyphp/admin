<div class="flex flex-col lg:flex-row gap-2">
    ...
    <div class="flex-1">
        <div class="relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="이메일, IP로 검색..."
    ...
    {{-- 상태 필터 --}}
    <div class="lg:w-32">
        <select wire:model.live="filter.action"
            ...
            <option value="captcha_success">✓ 성공</option>
            <option value="captcha_failed">✗ 실패</option>
            <option value="captcha_missing">⚠ 미입력</option>
        </select>
    </div>

    {{-- 기간 필터 --}}
    <div class="lg:w-28">
        <select wire:model.live="filter.days"
            class="block w-full h-8 px-2.5 text-xs border border-gray-200 bg-white rounded focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out appearance-none cursor-pointer">
    ...
    <div class="mt-2 flex items-center gap-2">
        ...
        @if (!empty($filter['action']))
            @php
                ...
                $label = $actionLabels[$filter['action']] ?? $filter['action'];
                $color = $actionColors[$filter['action']] ?? 'gray';
            @endphp
            <span
                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-{{ $color }}-50 text-{{ $color }}-700 border border-{{ $color }}-200">
                {{ $label }}
                <button wire:click="$set('filter.action', '')" class="ml-1.5 hover:opacity-75 focus:outline-none">
                    <svg class="h-2.5 w-2.5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
                            d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                            clip-rule="evenodd" />
